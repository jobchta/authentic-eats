...
import { regions, cuisines } from "@/data/food-data";
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-12">
          {regions.map((region) => (
            ...
            >
              <p className={`font-display text-lg font-bold ${activeRegion === region.name ? "text-primary-foreground" : "text-foreground"}`}>
                {region.name}
              </p>
              <p className={`font-body text-xs mt-1 ${activeRegion === region.name ? "text-primary-foreground/70" : "text-muted-foreground"}`}>
                {region.dishCount.toLocaleString()} dishes · {region.restaurantCount.toLocaleString()} spots
              </p>
              <p className={`font-body text-[10px] mt-0.5 ${activeRegion === region.name ? "text-primary-foreground/50" : "text-muted-foreground/60"}`}>
                Top: {region.topCity}
              </p>
            </button>
          ))}
          ...
          {filtered.map((cuisine, i) => (
            ...
            >
              ...
              <div className="text-center">
                <p className="font-display text-xs font-semibold text-foreground">{cuisine.name}</p>
                <p className="font-body text-[10px] text-muted-foreground">{cuisine.count.toLocaleString()}</p>
              </div>
            </motion.button>
          ))}
        </motion.div>
